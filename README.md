...
Our model is based on the operations of a hypothetical Pet Adoption Agency. The Shelter Locations are the main part of the data model, where many of the different aspects of a pet adoption agency are located.The ShelterLocation entity contains the unique identifier along with its name, address, email, and assigned supervisor. A single shelter location can have many employees, as represented by a one to many relationship between the ShelterLocations and Employees entities. Each employee has its own unique identifier along with their name, role, email, what shelter they work for, and what supervisor they report to. A shelter location will have one shelter supervisor that oversees operations at the shelter and is in charge of many employees.
